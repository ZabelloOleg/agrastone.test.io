...
function selectCustom() {
	jQuery('.drop-select').select2({
		templateResult: formatState, 
		templateSelection: formatState, 
		minimumResultsForSearch: -1
	});

	$(".drop-select").val(localStorage.getItem('language')).trigger('change');

	$('.drop-select').on('select2:select', function (e) {
...
$('.animated-input').focus(function(){
	$(this).parents('.animated-row').addClass('focused');
});
...
$('.animated-input').blur(function(){
	...
	if ( inputValue == "" ) {
		...
		$(this).parents('.animated-row').removeClass('focused');
	} else {
...
function swiperSlider() {
	var swiper = new Swiper(".deliverables-slider", {
...
function initMobileNav() {
	jQuery('body').mobileNav({
		...
		menuOpener: '.nav-opener, .close-nav'
	});
}
